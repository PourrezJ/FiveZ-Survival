...
namespace FiveZ.Entities
{
    ...
    public class Ped : Entity
    {
        ...
        public Ped(PedModel model, Position position, float heading, IPlayer owner = null, int dimension = Main.GlobalDimension) : base(position, dimension)
        {
            Model = model;
            Heading = heading;
            Owner = owner;
            NetworkEntity = AltNetworking.CreateEntity(position.ConvertToEntityPosition(), dimension, Main.StreamDistance, Export(), StreamingType.EntityStreaming);
        }

        public static Ped CreateNPC(PedModel pedHash, Position startPosition, float facingAngle, short dimension = Main.GlobalDimension, IPlayer owner = null)
        {
        ...
        public override Dictionary<string, object> Export()
        {
            ...
            data["entityType"] = EntityType.Ped;
